...
/*
    ...
    public boolean setPublicA(BigInteger publicA, String sha) {
        ...
        System.out.println("[Сервер] Получил Публичный ключ DH1 от клиента.");
        System.out.println("[Сервер] Началась проверка подлинности DH1");
        if(getSHA256(publicA.toString()).equals(sha))
        {
            System.out.println("[Сервер] Хеши сошлись. DH1 подлинный.");
            System.out.println("[Сервер] Общий секрет =");

            sharedKeyB = publicA.modPow(secretB,primeValue);
            System.out.println(sharedKeyB);
        }else
        {
            System.out.println("[Сервер] Хеши не сошлись. Кто-то подделал DH1.");
            return false;
        }
    ...
    public DHServer(BigInteger primeValue, BigInteger generatorValue) throws Exception{
